fetch_precip_bins <- function(precip_config){
  ...
  breaks <- precip_config$precip_breaks
  cols <- precip_config$precip_cols
  right_break <- c(breaks[-1], 10000)
  #right_break <- c(seq(bin, by = bin, length.out = (length(cols) - 1)), Inf)
  left_break <- breaks
  #left_break <- seq(0, by = bin, length.out = length(cols))
  #left_break[1] <- 1 # dropping less than 1 inch

  #browser()

  data.frame(left_break  = left_break, right_break = right_break, col = cols[1:length(left_break)]) %>%
